...
bool Adafruit_PN532::begin() {
  if (spi_dev) {
    ...
    ser_dev->setTimeout(
        100); // readBytes timeout: 10ms yeterli (115200 baud'da 64byte=5.6ms)
...
bool Adafruit_PN532::SAMConfig(void) {
  ...
  pn532_packetbuffer[3] =
      (ser_dev) ? 0x00 : 0x01; // HSU: no IRQ, I2C/SPI: use IRQ
...
bool Adafruit_PN532::waitready(uint16_t timeout) {
  uint16_t timer = 0;
  uint8_t poll_interval = (ser_dev) ? 10 : 10; // UART: 1ms, I2C/SPI: 10ms
  while (!isready()) {
    if (timeout != 0) {
      timer += poll_interval;
      if (timer > timeout) {
...
void Adafruit_PN532::writecommand(uint8_t *cmd, uint8_t cmdlen) {
  if (spi_dev) {
    ...
    if (i2c_dev) {
      ...
      // HSU: Onceki komuttan kalan veriyi temizle
      while (ser_dev->available())
        ser_dev->read();
