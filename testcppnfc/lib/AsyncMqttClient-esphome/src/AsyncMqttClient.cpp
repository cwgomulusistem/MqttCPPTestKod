...
,_pendingDisconnectReason(AsyncMqttClientDisconnectReason::TCP_DISCONNECTED)
, _host(nullptr)
, _useIp(false)
...
void AsyncMqttClient::_onDisconnect(AsyncClient* client) {
  (void)client; // Bu parametre hala doğrudan kullanılmıyor gibi duruyor.
  AsyncMqttClientDisconnectReason final_reason; // Kullanıcının callback'ine gidecek son neden

  // Önce _connected durumunu kontrol et. Eğer _connected true ise ve _onDisconnect çağrıldıysa,
  // bu genellikle beklenmedik bir TCP kopmasıdır (ve _pendingDisconnectReason TCP_DISCONNECTED olmalı).
  // Eğer _connected false ise ve _onConnAck'ten geliyorsak, _pendingDisconnectReason set edilmiş olmalı.

  // İstemci tarafında belirlenen spesifik hataları (bellek, TLS) önceliklendir.
  if (_connectPacketNotEnoughSpace) {
    final_reason = AsyncMqttClientDisconnectReason::ESP8266_NOT_ENOUGH_SPACE;
  } else if (_tlsBadFingerprint) {
    final_reason = AsyncMqttClientDisconnectReason::TLS_BAD_FINGERPRINT;
  } else {
    // Yukarıdaki özel durumlar değilse, _onConnAck'ten (veya başka bir yerden) gelen
    // _pendingDisconnectReason'ı kullan.
    final_reason = _pendingDisconnectReason;
  }

  // _connected durumunu sakla, _clear() bunu false yapacak.
  // Bu, callback'e "clientWasConnected" gibi bir bilgi vermek için kullanılabilir,
  // ama mevcut AsyncMqttClientDisconnectReason bunu doğrudan içermiyor.
  // bool clientWasActuallyConnected = _connected;

  _clear(); // MQTT durumunu temizle (bu _connected'ı false yapar)

  // Bir sonraki olası bağlantı kesilmesi için _pendingDisconnectReason'ı
  // varsayılan bir değere (örn: TCP_DISCONNECTED) sıfırlayın.
  // Bu, eğer bir sonraki kopma _onConnAck'ten kaynaklanmıyorsa (örn. normal bir ağ kopması),
  // mantıklı bir varsayılan sağlar.
  _pendingDisconnectReason = AsyncMqttClientDisconnectReason::TCP_DISCONNECTED;

  // Kullanıcı callback'lerini çağır
  if (!_onDisconnectUserCallbacks.empty()) {
    for (auto callback : _onDisconnectUserCallbacks) {
      callback(final_reason); // Kullanıcıya belirlenen son nedeni gönder
    }
  }

  // ESP32'de `connectReturnCode`'u (daha doğrusu onun `AsyncMqttClientDisconnectReason` karşılığını) görmek için:
  // `onDisconnect` callback'inizi (kullanıcı kodunuzdaki) şu şekilde yazdırabilirsiniz:
  /*
  void onMyMqttDisconnect(AsyncMqttClientDisconnectReason reason) {
    Serial.print("Disconnected. Reason (enum value): ");
    Serial.print(static_cast<int>(reason)); // Enum'un sayısal değerini yazdırır
    Serial.print(" - ");

    // Daha okunabilir bir çıktı için (bu kısmı kendi kodunuza ekleyebilirsiniz)
    if (reason == AsyncMqttClientDisconnectReason::TCP_DISCONNECTED) Serial.println("TCP Disconnected");
    else if (reason == AsyncMqttClientDisconnectReason::MQTT_UNACCEPTABLE_PROTOCOL_VERSION) Serial.println("MQTT Unacceptable Protocol Version (1)");
    else if (reason == AsyncMqttClientDisconnectReason::MQTT_IDENTIFIER_REJECTED) Serial.println("MQTT Identifier Rejected (2)");
    else if (reason == AsyncMqttClientDisconnectReason::MQTT_SERVER_UNAVAILABLE) Serial.println("MQTT Server Unavailable (3)");
    else if (reason == AsyncMqttClientDisconnectReason::MQTT_BAD_USER_CREDENTIALS) Serial.println("MQTT Bad User Credentials (4)");
    else if (reason == AsyncMqttClientDisconnectReason::MQTT_NOT_AUTHORIZED) Serial.println("MQTT Not Authorized (5)"); // <<< BURADA GÖRÜRSÜNÜZ
    else if (reason == AsyncMqttClientDisconnectReason::ESP8266_NOT_ENOUGH_SPACE) Serial.println("Not Enough Space");
    else if (reason == AsyncMqttClientDisconnectReason::TLS_BAD_FINGERPRINT) Serial.println("TLS Bad Fingerprint");
...
/* void AsyncMqttClient::_onDisconnect(AsyncClient* client) {
  // (void)client; // Bu satırı kaldırın veya yorumlayın
  AsyncMqttClientDisconnectReason reason_code; // Değişken adını 'reason' yaparsak aşağıdaki reason ile karışır,
                                             // bu yüzden 'reason_code' veya benzeri bir isim kullanalım.
                                             // Ya da callback'e giden 'reason' ile aynı ismi kullanıp
                                             // en sonda ona göre atama yapalım.
                                             // Orijinal kodda 'reason' isminde bir değişken zaten vardı,
                                             // onu kullanalım ve kapsam sorununa dikkat edelim.

  Serial.println(F("AsyncMqttClient::_onDisconnect called.")); // F() makrosu stringleri flash'ta tutar
  if (client) {
    Serial.print(F("  Client Pointer: 0x"));
    Serial.println(reinterpret_cast<uintptr_t>(client), HEX);

    // AsyncClient'ın public metotlarını kontrol edelim.
    // Genellikle 'connected()' veya 'state()' gibi metotlar olabilir.
    // _onDisconnect anında 'connected()' muhtemelen false dönecektir.
    Serial.print(F("  Client->connected(): "));
    Serial.println(client->connected() ? F("Yes (unlikely at disconnect)") : F("No"));

    // AsyncClient'ın boş olup olmadığını kontrol eden bir metod (genellikle AsyncTCP'de queue'lar için)
    // Serial.print(F("  Client->space(): "));
    // Serial.println(client->space()); // Ne kadar yazma alanı olduğu

    // Uzak IP ve port bilgileri AsyncClient üzerinden doğrudan alınamayabilir.
    // Bu bilgiler genellikle bağlantı kurulurken (_host, _ip, _port üyelerinde) saklanır
    // veya AsyncTCP'nin kendisinden istenir.
    // _onDisconnect anında bu client nesnesi hala _client üye değişkenimizle aynı mı kontrol edebiliriz.
    if (client == &_client) {
      Serial.println(F("  Client matches _client member."));
      Serial.print(F("  Broker Host/IP (from MqttClient config): "));
      if (_useIp) {
        Serial.println(_ip.toString());
      } else {
        Serial.println(_host ? _host : "N/A");
      }
      Serial.print(F("  Broker Port (from MqttClient config): "));
      Serial.println(_port);
    } else {
      Serial.println(F("  Client pointer does NOT match _client member. This is unexpected."));
    }

  } else {
    Serial.println(F("  Client pointer is null."));
  }

  // Disconnect nedenini belirleme
  AsyncMqttClientDisconnectReason reason; // Bu, callback'e gidecek olan 'reason'
  if (_connectPacketNotEnoughSpace) {
    reason = AsyncMqttClientDisconnectReason::ESP8266_NOT_ENOUGH_SPACE;
    Serial.println(F("  Disconnect Reason: ESP8266_NOT_ENOUGH_SPACE"));
  } else if (_tlsBadFingerprint) {
    reason = AsyncMqttClientDisconnectReason::TLS_BAD_FINGERPRINT;
    Serial.println(F("  Disconnect Reason: TLS_BAD_FINGERPRINT"));
  } else {
    // Diğer durumlar: Genellikle TCP bağlantı kopmasıdır.
    // AsyncMqttClient.cpp içinde _disconnectReason diye bir üye değişken var mı
    // veya _onDisconnect çağrılmadan önce bu durum set ediliyor mu kontrol edilmeli.
    // Eğer _onDisconnect çağrıldığında _client.state() veya benzeri bir bilgi
    // varsa (örn: AsyncTCPError), bu da bir ipucu verebilir.
    // Şimdilik genel bir TCP kopması varsayalım.
    reason = AsyncMqttClientDisconnectReason::TCP_DISCONNECTED;
    Serial.println(F("  Disconnect Reason: TCP_DISCONNECTED (or other based on internal state)"));
  }

  _clear(); // MQTT durumunu temizle

  // Kullanıcı callback'lerini çağır
  if (!_onDisconnectUserCallbacks.empty()) {
    Serial.print(F("  Invoking "));
    Serial.print(_onDisconnectUserCallbacks.size());
    Serial.println(F(" user disconnect callback(s)."));
    for (auto callback : _onDisconnectUserCallbacks) {
      callback(reason);
    }
  } else {
    Serial.println(F("  No user disconnect callbacks registered."));
  }
  Serial.println(F("AsyncMqttClient::_onDisconnect finished."));
}
 */
...
void AsyncMqttClient::_onConnAck(bool sessionPresent, uint8_t connectReturnCode) {
  ...
  Serial.println("AsyncMqttClient::_onConnAck called.");
  Serial.print("  Connect Return Code: ");
  ...
  if (connectReturnCode == 0) {
    ...
    for (auto callback : _onConnectUserCallbacks) callback(sessionPresent);
      ...
      _connected = false; // Bağlı değiliz

    // connectReturnCode'u AsyncMqttClientDisconnectReason'a çevir
    switch (connectReturnCode) {
      case 1: // Unacceptable protocol version
        _pendingDisconnectReason = AsyncMqttClientDisconnectReason::MQTT_UNACCEPTABLE_PROTOCOL_VERSION;
        break;
      case 2: // Identifier rejected
        _pendingDisconnectReason = AsyncMqttClientDisconnectReason::MQTT_IDENTIFIER_REJECTED;
        break;
      case 3: // Server unavailable
        _pendingDisconnectReason = AsyncMqttClientDisconnectReason::MQTT_SERVER_UNAVAILABLE;
        break;
      case 4: // Bad user name or password
        _pendingDisconnectReason = AsyncMqttClientDisconnectReason::MQTT_MALFORMED_CREDENTIALS;
        break;
      case 5: // Not authorized
        _pendingDisconnectReason = AsyncMqttClientDisconnectReason::MQTT_NOT_AUTHORIZED; // <<< İSTEDİĞİMİZ YER
        break;
      default:
        // Bilinmeyen bir MQTT v3.1.1 hata kodu (normalde 0-5 arasıdır)
        // Genel bir hata olarak işaretleyebiliriz veya kütüphanenin daha spesifik
        // bir "bilinmeyen MQTT hatası" enum değeri varsa o kullanılır.
        _pendingDisconnectReason = AsyncMqttClientDisconnectReason::TCP_DISCONNECTED; // Güvenli bir varsayılan
        break;
    }
